...
const createVideo = async (req, res) => {
    try {
        ...
        const requiredFields = { title, description, category, videoUrl, thumbnail };
        ...
        const thumbnailResult = await uploadOnCloudinary(thumbnail);

        if (!result || !thumbnailResult) {
            return res.status(400).json({ message: "Error while uploading file on cloudinary" });
        }
